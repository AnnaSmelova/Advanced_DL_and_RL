# Источник исходного кода: https://github.com/openai/gym/blob/master/gym/envs/toy_text/blackjack.py
...
class BlackjackDoubleCountEnv(gym.Env):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, render_mode: Optional[str] = None, natural=False, sab=False):
        self.action_space = spaces.Discrete(3)  # in double version we have 3 actions
    ...
    def count(self, *cards):
        # Система система подсчёта «Плюс-минус» - https://ru.wikipedia.org/wiki/%D0%91%D0%BB%D1%8D%D0%BA%D0%B4%D0%B6%D0%B5%D0%BA
        for card in cards:
    ...
    def step(self, action):
        ...
        if action == 1:  # hit: add a card to players hand and return
            ...
            if self.sab and is_natural(self.player) and not is_natural(self.dealer):
                ...
                reward = 1.5
        else:  # double: get one card and stop the game with double reward
            card = draw_card(self.np_random)
            self.player.append(card)
            self.count(card)
            terminated = True
